...
while (np.mod(MedSize,2) == 0):
    MedSize = raw_input("Enter the median window size (enter an odd number as 101):  ")
    MedSize = int(MedSize)
...
FolderOut = FolderNames[len(FolderNames)-1] + FileName + 'BckgSubs'
Bckg_FN = FolderNames[len(FolderNames)-1] + FileName +'Bckg'
